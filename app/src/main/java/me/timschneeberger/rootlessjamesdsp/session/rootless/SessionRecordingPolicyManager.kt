...
class SessionRecordingPolicyManager(private val context: Context) {
    ...
    @Synchronized fun update(dump: ISessionPolicyInfoDump)
    {
        if(isDisposing) {
        ...
        {
            changeCallbacks.toList().forEach {
                it.onSessionRecordingPolicyChanged(HashMap(sessionPolicyList), isMinorUpdate)
            }
        }
    }
    ...
    @Synchronized fun getRestrictedUids(): Array<Int> = sessionPolicyList.values
        .filter { it.isRestricted }
        .filter { it.uid > 0 }
        .map { it.uid }
        .toTypedArray()

    @Synchronized fun registerOnRestrictedSessionChangeListener(changeListener: OnSessionRecordingPolicyChangeListener) {
        changeCallbacks.add(changeListener)
    }

    @Synchronized fun unregisterOnRestrictedSessionChangeListener(changeListener: OnSessionRecordingPolicyChangeListener) {
